...
def CBAM_block(input_layer, filter_num, reduction_ratio=32, name=None):
	...
	dense2 = layers.Dense(filter_num, activation='relu', name=name + "_Chanel_FC_2")
	...
	# SPATIAL ATTENTION
	avg_pool = tf.reduce_mean(input_layer, axis=axis, keepdims=True)
	max_pool = tf.reduce_max(input_layer, axis=axis, keepdims=True)

	spatial = layers.concatenate([avg_pool, max_pool], axis=axis)

	# K = 7 achieves the highest accuracy
	spatial = layers.Conv2D(1, kernel_size=(7, 7), strides=1, padding='same', name=name + "_Spatial_Conv2D")(spatial)
	...
	CBAM_out = tf.keras.layers.Add()([CBAM_out, input_layer])
